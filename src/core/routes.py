...
try:
    from ..utils.spend_score_advanced import calculate_advanced_spend_score, spend_score_engine
    # Import missing functions from legacy engine
    from ..utils.spend_score_engine import get_enhanced_analysis, get_score_label, get_score_color, calculate_spend_score
    logging.info("✅ Advanced SpendScore™ engine loaded successfully")
    ADVANCED_SCORING = True
except ImportError:
    from ..utils.spend_score_engine import calculate_spend_score, get_score_label, get_score_color, get_enhanced_analysis
    logging.warning("⚠️ Using legacy SpendScore engine")
    ADVANCED_SCORING = False
from ..services.pdf_generator import generate_report_pdf
from ..utils.clone_verifier import verify_project_integrity

# Initialize sample data
init_sample_data()

# Import comprehensive static APIs for SaaS platform
try:
    from ..utils import dynamic_apis
    logging.info("✅ Dynamic APIs loaded successfully")
...
def serve_logo_svg():
    ...
    return send_from_directory('dynamic/assets/images', 'verocta-logo.png')
...
def login():
    ...
    try:
        ...
        try:
            from auth import create_refresh_token as _create_refresh_token
        except Exception:
            _create_refresh_token = None

        refresh_token = None
        if _create_refresh_token:
            try:
                refresh_token = _create_refresh_token(identity=email)
            except Exception:
                refresh_token = None
...
@app.route('/api/spend-score', methods=['GET', 'POST'])
@jwt_required()
def get_spend_score():
...
# Simple OpenAPI spec (subset) for Swagger UI
OPENAPI_SPEC = {
    "openapi": "3.0.3",
    "info": {
        "title": "VeroctaAI API",
    ...
    "paths": {
        ...
        "/api/upload": {"post": {"summary": "Upload CSV", "responses": {"200": {"description": "OK"}}}},
        "/api/reports": {"get": {"summary": "List reports", "responses": {"200": {"description": "OK"}}}}
    }
}
...
def swagger_ui():
    ...
    <html>
      <head>
        <meta charset="utf-8" />
        <title>VeroctaAI API Docs</title>
    ...
    <body>
        <div class="container">
            ...
            <div class="section">
                ...
                <p>This is a backend-only API service. Frontend assets have been removed.</p>
                <p>Use the API endpoints directly; CORS is configured for common origins.</p>
            </div>
        </div>
    </body>
    </html>
...
def serve_dynamic(filename):
    """Dynamic file route disabled in backend-only mode.

    Return 404 for non-API static file requests to avoid referencing removed frontend files.
    """
    # Only allow API routes handled elsewhere
    if filename.startswith('api/'):
        return "API route not found", 404

    return jsonify({'error': 'Dynamic files and frontend are not available in this backend-only repository.'}), 404
...
def not_found(e):
    ...
    # For other routes, return a simple JSON message in backend-only mode
    return jsonify({'error': 'Resource not found', 'note': 'Frontend removed in backend-only repo'}), 404
...
def server_error(e):
    ...
    # For other routes, return JSON note that frontend is removed
    return jsonify({'error': 'Internal server error', 'note': 'Frontend removed in backend-only repo'}), 500