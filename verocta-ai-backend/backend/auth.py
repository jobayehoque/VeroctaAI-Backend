...
def init_auth(app):
    ...
    app.config['JWT_ACCESS_TOKEN_EXPIRES'] = timedelta(minutes=int(os.environ.get('JWT_ACCESS_MINUTES', 60)))
    app.config['JWT_REFRESH_TOKEN_EXPIRES'] = timedelta(days=int(os.environ.get('JWT_REFRESH_DAYS', 7)))
    # Support tokens via headers (default). Cookies can be enabled if desired by changing JWT_TOKEN_LOCATION
    app.config.setdefault('JWT_TOKEN_LOCATION', ['headers'])

    jwt = JWTManager(app)

    # Register blocklist checker
    @jwt.token_in_blocklist_loader
    def check_if_token_revoked(jwt_header, jwt_payload):
        jti = jwt_payload.get('jti')
        try:
            # If redis client, use exists
            if hasattr(token_blocklist, 'exists'):
                return token_blocklist.exists(jti)
            else:
                return jti in token_blocklist
        except Exception:
            return False

    # Optional callbacks for helpful error messages
    ...
    def missing_token_callback(error):
        return jsonify({'error': 'Missing Authorization Header', 'message': error}), 401

    return jwt


def revoke_token(jti):
    """Revoke a token by jti (add to blocklist)."""
    try:
        if hasattr(token_blocklist, 'set') and not isinstance(token_blocklist, set):
            # Redis client
            # Use a TTL roughly equal to refresh token expiry to avoid permanent storage
            token_blocklist.set(jti, 'revoked')
        else:
